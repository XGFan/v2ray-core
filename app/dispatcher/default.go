	...
	if forcedOutboundTag := session.GetForcedOutboundTagFromContext(ctx); forcedOutboundTag != "" {
		...
		if route, err := d.router.PickRoute(routing_session.AsRoutingContext(ctx)); err == nil {
			...
			if h := d.ohm.GetHandler(tag); h != nil {
				newError("taking detour tag: [", tag, "] for [", destination, "]").WriteToLog(session.ExportIDToError(ctx))
				handler = h
			} else {
				newError("non existing tag: [", tag, "] for [", destination, "]").AtWarning().WriteToLog(session.ExportIDToError(ctx))
			}
		} else {
